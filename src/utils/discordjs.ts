import { PermissionFlagsBits, type Channel, type GuildBasedChannel, type TextBasedChannel } from "discord.js";
...
export const checkIsValidTextChannel = (channel: Channel): channel is GuildBasedChannel & TextBasedChannel => {
	...
	if (!channel.isTextBased()) throw new UserError(`${channel} must be a text channel`);

	const clientPermissionsInChannel = channel.permissionsFor(channel.client.user);
